...
Renders Class_03_Understanding_Data.pptx to a PDF by replaying the exact
same slide-building calls from generate_presentation.py against a
lightweight in-memory shim (instead of python-pptx's real Presentation
object), then rasterizing each slide with Pillow into one multi-page PDF.
...
Class 01's PRESENTATION_README.md §9 for visual QA rendering — verified
again for this class: PowerPoint COM instantiates but returns a broken
proxy with no interactive desktop session available, and no LibreOffice/
unoconv/docx2pdf install exists on this machine). This script is the
pure-Python fallback used instead.
...
def main():
    ...
    out_pdf = os.path.join(here, "Class_03_Understanding_Data.pdf")
